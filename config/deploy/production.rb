...
namespace :db do
  task :dump_db do
    on roles (:app) do
      ...
      execute "AWS_ACCESS_KEY_ID=#{ENV['AWS_ACCESS_KEY_ID']} AWS_SECRET_ACCESS_KEY=#{ENV['AWS_SECRET_ACCESS_KEY']} TRAVIS_BUILD_NUMBER=#{ENV['$TRAVIS_BUILD_NUMBER']} ruby #{deploy_to}/current/db_upload.rb"
    end
  end
end